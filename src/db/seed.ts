...
// Provisiona (idempotente) la encuesta de onboarding inicial. Se puede invocar
// de forma aislada al arranque sin ejecutar el seed completo (que crea usuarios
// de prueba). Si la encuesta existe pero esta soft-deleted, la restaura.
export const ensureOnboardingSurvey = async () => {
  ...
  const [existente] = await db
    .select({ id: schema.encuestas.id, deleted_at: schema.encuestas.deleted_at })
  ...
  if (existente.deleted_at) {
    await db
      ...
      .where(eq(schema.encuestas.id, existente.id));
    console.log('Encuesta ONBOARDING_INICIAL restaurada');
    return;
  }
